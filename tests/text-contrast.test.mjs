 ...
 * 路口轉向那一支的 `.table-scroll td small` 在**白底**上只有 2.92:1，
 * 整份樣式表掃出 43 處低於 4.5:1。同一把尺拿來量這一支：
 * 這一支在 v20.62 已經整體調深過，只剩 2 處（4.14 與 4.32），一併補上。
 * 這一支測試留著，是為了讓日後新增的樣式自動被納入。
 *
 ...
 */
const DARK_GROUND = [
  ["sidebar", "側欄 --side #0f3d31"],
  ["side-nav", "側欄導覽，同上"],
  ["side-note", "側欄底部說明，同上"],
  ["project-card", "側欄的計畫卡片，同上"],
  ["project-code", "側欄的計畫編號，同上"],
  /*
   * ⚠️ 2026-09-17：頁首那條藍色橫幅整條拿掉（使用者回報它占版面又會捲走），
   *   .topbar / .user-chip / .eyebrow / .version-badge / .avatar 已不再渲染。
   *   取而代之的是側欄左上角的 .brand 與側欄底部的 .side-foot，
   *   兩者的底色都是側欄的 --side，一樣要豁免白底掃描。
   */
  ["brand", "側欄左上角的招牌，底是 --side #0f3d31"],
  ["brand-version", "招牌上的版號徽章，同上"],
  ["side-foot", "側欄底部的使用者身分，同上"],
  [".toast", "深色浮出提示 --navy"],
  [".busy", "半透明深色遮罩"],
  ["table-wrap th", "表頭，底是 --teal #0c6f70（白字實測 5.96:1）"],
  ["button.primary", "主要按鈕，底是 --teal"],
  ["button.split", "匯出分割鈕，底是 #0c6f70"],
];
/*
 * 有色底上的文字，逐組釘住實際的前景／背景。
 *
 * ⚠️ 上面那張豁免表是拿**白底**掃整份樣式表，對「深底上的白字」量不準，
 *   所以那種只能豁免。但**豁免不等於不用管**——那正是最容易出事的地方：
 *   姊妹專案（路口轉向）就踩過一次，一個徽章用了 #d99a19 配白字、
 *   實測只有 2.45:1，字幾乎融進底色，而整份掃描因為豁免完全不會紅。
 *   所以凡是靠豁免過關的有色底，都要在這裡補一組明確的前景／背景。
 */
const ON_COLOR = [
  /* 側欄的小分頁收合鈕（2026-09-13 新增）。底是側欄的 --side #0f3d31。 */
  ["側欄收合鈕", "#eaf4fa", "#0f3d31"],
  ["側欄收合鈕（滑鼠移上去）", "#ffffff", "#0f3d31"],
];
...
test("淺底上的每一個文字色都要 ≥ 4.5:1（整份樣式表掃過，不是抽樣）", () => {
  ...
  assert.ok(
    inspected >= 30,
    `只掃到 ${inspected} 個文字色，太少——選擇器解析是不是壞了？（這一支的樣式表是壓縮過的單行，解析更容易出錯）`,
  );
  assert.deepEqual(
    failures,
    [],

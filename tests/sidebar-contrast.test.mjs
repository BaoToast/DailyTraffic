 ...
 *    本程式（全日交通量）的那一個另外從 globals.css 讀出來比對，
...
test("側欄字級要嚴格遞減：大分頁 > 小分頁", () => {
   ...
   *
   * ⚠️ 這一支的側欄結構上只有**兩層**（沒有中間的分類標題那一層），
   *   所以只驗「大分頁 > 小分頁」。另外兩支是三層，各自有自己的測試。
   *
   * ⚠️ 這一條**取代**了舊的「主項目 16px、子項目 14px」。
   *   釘死數字的話，日後整體放大一級就會無謂地轉紅；
   *   而只驗「有沒有設 font-size」則完全擋不住「兩層一樣大」——
   *   2026-09-15 使用者附圖說「感覺兩個文字一樣大」時，實測差的正是 0px，
   *   原因是 `.side-nav-item`（0,1,0）被 `.side-nav button`（0,1,1）壓過去。
   *
   * ⚠️ 所以選擇器要照抄 CSS 裡**實際生效**的那一條（`.side-nav .side-nav-item`）。
   *   比對一個被壓過去、根本沒生效的宣告，量到的是假的。
   */
  ...
  const page = sizeOf(".side-nav button", "大分頁");
  const sub = sizeOf(".side-nav .side-nav-item", "小分頁");
  assert.ok(
    page > sub,
    `大分頁 ${page}px 沒有比小分頁 ${sub}px 大——兩層看起來一樣，分不出層級`,
  );
  /*
   * ⚠️ 差 1px 在深色底上幾乎看不出來，所以要求至少差 2px。
   */
  assert.ok(
    page - sub >= 2,
    `兩層字級是 ${page}／${sub}px，至少要差 2px 才看得出來`,
  );
});

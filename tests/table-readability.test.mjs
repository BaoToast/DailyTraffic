...
test("斑馬紋的底色必須淡到「分得出行」而不是「看起來有顏色」", () => {
  const zebra = colorOf(".table-wrap tbody tr:nth-child(even)");
...
test("斑馬列與 hover 列上的文字對比都要過 AA", () => {
  const zebra = colorOf(".table-wrap tbody tr:nth-child(even)");
  const hover = colorOf(".table-wrap tbody tr:hover");
...
test("數字欄要用等寬數字，這一項不加任何顏色卻解決同一個問題", () => {
  assert.match(
    css,
    /\.table-wrap (td|th)[\s\S]{0,80}font-variant-numeric:\s*tabular-nums/,

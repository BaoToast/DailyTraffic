...
目前版本：**v16.1（2026-08-14）**

本版同時支援原有雙向路段格式，以及 3～7 支線的路口轉向全日交通量格式。支援拖曳批次匯入、路段／路口自動切換明細表、各車種左轉／直行／右轉PCU係數，以及支線名稱、角度、轉向圖與人工修正。路口 A／B／C…依原始報告的來源支線彙整，不再依目的地重新分配，因此可直接核對原表各支線車輛數。

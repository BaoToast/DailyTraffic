...
const dist = join(root, "github-pages", ".tryout");
const indexPath = join(dist, "index.html");
if (!existsSync(indexPath))
  throw new Error(
    "找不到 github-pages/.tryout/index.html——請先執行 npm run build:tryout",
  );
 ...
 *
 * ⚠️ 2026-09-14 實測抓到（把產出的試用版真的用 file:// 打開、走到手冊那一頁
 *   去量那顆按鈕的 href，不是讀原始碼猜的）：
 *   「新手使用手冊 PDF」的 href 是 ./manuals/全日交通流量程式手冊_vX.Y.pdf。
 *   單檔試用版旁邊沒有那個資料夾，按下去在 file:// 下**不會有任何錯誤訊息**，
 *   畫面毫無反應——使用者只會覺得按鈕壞了，而且不會來問。
 *
 *   交通服務水準早就踩過同一顆雷並修好了（它把手冊嵌成 data URI），
 *   本支與路口轉向卻一路沒有補上——正是使用者說的
 ...
 * 實測（headless Chromium，各量 7 次取平均）：本支 5.64 MB → 2.19 MB、
 * DOMInteractive 318ms → 161ms（−49%）。手冊 PDF 佔整個試用版的 61%。
...
html = html.replace(manualLink, (whole, file) => {
  ...
  const full = join(dist, "manuals", name);
...
for (const name of manualFiles) {
  ...
  copyFileSync(join(dist, "manuals", name), target);

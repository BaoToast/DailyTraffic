...
/*
 * ────────────────────────────────────────────────────────────────
 *  對 xlsx（SheetJS 0.20.3）第三方工作簿解析的縱深防護
 * ────────────────────────────────────────────────────────────────
 *
 * 官方修正版 0.20.3 由專案內 vendor tarball 安裝，不依賴安裝當下能否連上
 * SheetJS CDN。即使上游問題已修正，仍保留以下防護，避免第三方工作簿利用
 * 未知解析缺口影響目前分頁。
 *
 ...
 *     使用者會以為那個檔案沒問題。
 *
 * 這些措施不取代套件修正，而是在已升級後再加一道防線：若仍有污染跡象，
 * 會從「無聲被污染」改成「當場中止並告知」。
 */
